...
# raw CSV columns are authored in (SBJ, HD, CFP) order, not (CFP, HD, SBJ) -
# this must match emit_raw's actual comma order, since values are assigned
# to emit_array positionally with no reordering
...
stan_data_post <- list(
  ...
  emit = emit_array,
  prior_only = 0,
  # see ethics_rel2_model.stan's comment on beta_prior_scale; sensitivity to
  # this choice is checked separately in prior_sensitivity_check.R
  beta_prior_scale = 2
)
...
print(param_summary)
write.csv(param_summary, "output/param_summary.csv", row.names = FALSE)
...
summary_prior_rel_minus1 <- data.frame(
  ...
  ci_lower = apply(prior_draws_rel_minus1, 2, quantile, 0.05),
  ci_upper = apply(prior_draws_rel_minus1, 2, quantile, 0.95),
...
summary_prior_rel_plus1 <- data.frame(
  ...
  ci_lower = apply(prior_draws_rel_plus1, 2, quantile, 0.05),
  ci_upper = apply(prior_draws_rel_plus1, 2, quantile, 0.95),
...
write.csv(summary_prior, "output/summary_prior.csv", row.names = FALSE)

prior_breaks <- seq(0, 1, 0.2)
...
p_prior <- ggplot(
  ...
  scale_y_continuous(limits = range(prior_breaks), breaks = prior_breaks) +
  labs(
    title = "Prior Distribution of Preferred Division Principles by Relationship",
    y = "Prior probability",
    x = NULL,
    color = NULL
  ) +
...
print(p_prior)
ggsave("output/plot_prior.png", p_prior, width = 10.5, height = 4.5, dpi = 150)

# ------------------------------------------------------------
# 12b. Prior comparison plot for all ethical principles
# difference = non-kin/non-friends - kin/friends
# ------------------------------------------------------------
...
prior_diff_breaks <- pretty(c(summary_prior_diff$ci_lower, summary_prior_diff$ci_upper))

p_prior_diff <- ggplot(summary_prior_diff, aes(x = ethical_principle, y = mean)) +
  geom_hline(yintercept = 0, linetype = 2, color = "gray50") +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.15) +
  scale_y_continuous(limits = range(prior_diff_breaks), breaks = prior_diff_breaks) +
  labs(
    title = "Prior Difference in Preferred Division Principle Probabilities",
...
print(p_prior_diff)
print(summary_prior_diff)
ggsave("output/plot_prior_diff.png", p_prior_diff, width = 10.5, height = 4.5, dpi = 150)
write.csv(summary_prior_diff, "output/summary_prior_diff.csv", row.names = FALSE)

# ------------------------------------------------------------
# 12c. Prior difference plot - point-cloud version (each point is one
# prior draw, semi-transparent) instead of mean + CI, to see whether the
# prior mass concentrates near 0 or is diffusely spread out
# ------------------------------------------------------------

prior_diff_long <- as.data.frame(prior_diff_draws) %>%
  pivot_longer(everything(), names_to = "ethical_principle", values_to = "diff") %>%
  mutate(
    ethical_principle = recode(ethical_principle, !!!pretty_labels),
    ethical_principle = factor(ethical_principle, levels = desired_order)
  )

prior_diff_points_breaks <- pretty(range(prior_diff_long$diff))

p_prior_diff_points <- ggplot(prior_diff_long, aes(x = ethical_principle, y = diff)) +
  geom_hline(yintercept = 0, linetype = 2, color = "gray50") +
  geom_jitter(width = 0.15, height = 0, size = 0.5, alpha = 0.05) +
  scale_y_continuous(limits = range(prior_diff_points_breaks), breaks = prior_diff_points_breaks) +
  labs(
    title = "Prior Difference in Preferred Division Principle Probabilities",
...
print(p_prior_diff_points)
ggsave("output/plot_prior_diff_points.png", p_prior_diff_points, width = 10.5, height = 4.5, dpi = 150)
...
write.csv(summary_post, "output/summary_post.csv", row.names = FALSE)

post_breaks <- pretty(c(summary_post$ci_lower, summary_post$ci_upper))
...
p_post <- ggplot(
  ...
  scale_y_continuous(limits = range(post_breaks), breaks = post_breaks) +
  labs(
    title = "Posterior Distribution of Preferred Division Principles by Relationship",
...
print(p_post)
ggsave("output/plot_post.png", p_post, width = 10.5, height = 4.5, dpi = 150)
...
diff_breaks <- pretty(c(summary_diff$ci_lower, summary_diff$ci_upper))

p_diff <- ggplot(summary_diff, aes(x = ethical_principle, y = mean)) +
  geom_hline(yintercept = 0, linetype = 2, color = "gray50") +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.15) +
  scale_y_continuous(limits = range(diff_breaks), breaks = diff_breaks) +
  labs(
    title = "Posterior Difference in Preferred Division Principle Probabilities",
    subtitle = "non-kin/non-friends minus kin/friends",
    x = NULL,
...
print(p_diff)
print(summary_diff)
ggsave("output/plot_diff.png", p_diff, width = 10.5, height = 4.5, dpi = 150)
write.csv(summary_diff, "output/summary_diff.csv", row.names = FALSE)
...
sim_results <- replicate(
  n_sim,
  simulate_once(subject_cf_counts, p_non)
)
...
coverage_check <- data.frame(
  category = c("both", "only kin/friends", "only non"),
  observed = as.numeric(obs[c("both", "only kin/friends", "only non")]),
  expected_under_null = as.numeric(colMeans(sim_results)[c("both", "only kin/friends", "only non")]),
  p_value_one_sided = c(p_both_low, p_only_kin_high, p_only_non_high)
)
write.csv(coverage_check, "output/coverage_null_test.csv", row.names = FALSE)
...
resource_diff_summary <- dat2 %>%
  group_by(relationship) %>%
  summarise(
    n = n(),
    prop_resource_diff = mean(resource_diff, na.rm = TRUE)
  )
print(resource_diff_summary)
write.csv(resource_diff_summary, "output/resource_diff_summary.csv", row.names = FALSE)

table_test <- table(dat2$relationship, dat2$resource_diff)

fisher_result <- fisher.test(table_test)
print(fisher_result)
capture.output(fisher_result, file = "output/resource_diff_fisher_test.txt")
